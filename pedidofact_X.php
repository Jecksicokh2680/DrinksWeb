...
function obtenerDatos($cnx, $nombreSucursal, $f_ini, $f_fin, $busqProd, $f_fac, $f_ped) {
    ...
    // Filtro específico por número de documento/pedido si se proporciona
    $condPedidoExtraF = ($f_ped != "") ? " AND FACTURAS.NUMERO LIKE '%".$cnx->real_escape_string($f_ped)."%'" : "";
    $condPedidoExtraP = ($f_ped != "") ? " AND PEDIDOS.NUMERO LIKE '%".$cnx->real_escape_string($f_ped)."%'" : "";

    $condFactura = $extraCond . ($f_fac != "" ? " AND T1.NOMBRES = '".$cnx->real_escape_string($f_fac)."' " : "") . $condPedidoExtraF;
    $condPedido  = $extraCond . ($f_fac != "" ? " AND T2.NOMBRES = '".$cnx->real_escape_string($f_fac)."' " : "") . $condPedidoExtraP;
...
$fPed = trim($_GET['nro_pedido'] ?? ''); // Captura del filtro de pedido
$fProd = $_GET['filtro_prod'] ?? '';
...
$rows = [];
if ($fSuc == '' || $fSuc == 'CENTRAL') $rows = array_merge($rows, obtenerDatos($mysqliCentral, 'CENTRAL', $f_ini, $f_fin, $fProd, $fVend, $fPed));
if ($fSuc == '' || $fSuc == 'DRINKS') $rows = array_merge($rows, obtenerDatos($mysqliDrinks, 'DRINKS', $f_ini, $f_fin, $fProd, $fVend, $fPed));
...
<body>
    <form method="GET" class="filtros">
        ...
        <input type="text" name="nro_pedido" placeholder="N° de Pedido / Documento" value="<?= htmlspecialchars($_GET['nro_pedido'] ?? '') ?>">
