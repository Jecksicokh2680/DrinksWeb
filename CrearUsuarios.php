...
/* ============================================================
   RESPUESTAS AJAX (SALIDA TEMPRANA)
============================================================ */
if (isset($_GET['ajax'])) {

    header('Content-Type: application/json');

    // ---- BUSCAR TERCEROS EN CENTRAL ----
    if ($_GET['ajax'] === 'usuarios') {
        $term = "%" . ($_GET['term'] ?? '') . "%";
        $stmt = $mysqliPos->prepare("
            SELECT nit, nombres, apellidos
            FROM terceros
            WHERE inactivo = 0
              AND (nit LIKE ? OR nombres LIKE ? OR apellidos LIKE ?)
            LIMIT 10
        ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardarUsuario'])) {
    ...
    if ($CedulaNit === '' || $NitEmpresa === '' || $NroSucursal === '') {
        ...
        // Validar tercero en Central
        $stmtT = $mysqliPos->prepare("
            SELECT 1
            FROM terceros
            WHERE nit = ? AND inactivo = 0
            LIMIT 1
        ");
        $stmtT->bind_param("s", $CedulaNit);
        $stmtT->execute();
        $stmtT->store_result();
        $existeTercero = $stmtT->num_rows > 0;
        $stmtT->close();

        if (!$existeTercero) {
            $msg = "⚠ El tercero no existe o está inactivo en Central.";
        } else {
            ...
            if ($existe) {
                ...
                if ($Password !== '') {
                    ...
                    $stmt = $mysqli->prepare("
                        ...
                        WHERE CedulaNit=?
                    ");
                    $stmt->bind_param("ssss", $NitEmpresa, $NroSucursal, $hash, $CedulaNit);
                } else {
                    $stmt = $mysqli->prepare("
                        UPDATE usuarios_Seguridad
                ...
                // CREAR
                if ($Password === '') {
                    $msg = "⚠ Debe asignar contraseña para un nuevo usuario.";
                } else {
                    $hash = password_hash($Password, PASSWORD_DEFAULT);
                    $stmt = $mysqli->prepare("
...
/* ============================================================
   DATOS PARA TABLA (SIN REFERENCIAS ❌)
============================================================ */
$empresas = $mysqli->query("
    SELECT Nit, NombreComercial
    FROM empresa
    WHERE Estado = 1
")->fetch_all(MYSQLI_ASSOC);

$usuariosLocal = $mysqli->query("
    SELECT u.CedulaNit, u.NitEmpresa, u.NroSucursal, u.Estado, e.NombreComercial
    FROM usuarios_Seguridad u
    LEFT JOIN empresa e ON u.NitEmpresa = e.Nit
")->fetch_all(MYSQLI_ASSOC);

/* 🔥 CORRECCIÓN CLAVE: SIN & */
foreach ($usuariosLocal as $k => $u) {
    ...
    $usuariosLocal[$k]['NombreCompleto'] = $r
        ? trim($r['nombres'] . ' ' . $r['apellidos'])
        : "<span class='text-danger'>No encontrado en Central</span>";
...
<style>
.card-header{background:#1a2a3a;color:#fff}
.list-group-item:hover{cursor:pointer;background:#f8f9fa}
</style>
</head>
...
<div class="card-header d-flex justify-content-between align-items-center">
    <strong>USUARIOS CON ACCESO AL SISTEMA</strong>
    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modalUsuario">
        ➕ CONFIGURAR ACCESO
    </button>
</div>
...
<tr>
<td><strong><?= $u['CedulaNit'] ?></strong></td>
<td><?= $u['NombreCompleto'] ?></td>
<td><?= $u['NombreComercial'] ?> (Suc <?= $u['NroSucursal'] ?>)</td>
<td>
...
<td>
<button class="btn btn-outline-primary btn-sm" onclick="editarUser('<?= $u['CedulaNit'] ?>')">
Editar
</button>
</td>
</tr>
...
<h5 class="modal-title">Acceso de Seguridad</h5>
...
<select name="NroSucursal" id="NroSucursalModal" class="form-select mb-2" required>
...
<button class="btn btn-dark w-100 fw-bold">GUARDAR</button>
</div>
...
fetch('?ajax=usuarios&term='+val)
.then(r=>r.json())
.then(d=>{
let ul=document.getElementById('resultadosBusqueda');
ul.innerHTML='';
d.forEach(i=>{
let li=document.createElement('li');
li.className='list-group-item small';
li.innerHTML=`<strong>${i.nit}</strong> - ${i.nombre}`;
li.onclick=()=>{
...
function cargarSucursales(nit){
fetch('?ajax=sucursales&nit='+nit)
.then(r=>r.json())
.then(d=>{
let s=document.getElementById('NroSucursalModal');
s.innerHTML='<option value="">Sucursal...</option>';
d.forEach(i=>{
s.innerHTML+=`<option value="${i.NroSucursal}">${i.Direccion}</option>`;
});
});
}

function editarUser(nit){
document.getElementById('busquedaTercero').value=nit;
document.getElementById('CedulaNitModal').value=nit;
document.getElementById('terceroSeleccionado').innerText='Editando: '+nit;
