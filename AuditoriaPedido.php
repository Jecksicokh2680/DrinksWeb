...
foreach ($rows as $r) {
    ...
    // Guardamos también el Barcode y la cantidad original para el traspaso de datos
    $pedidos[$doc]['ITEMS'][] = [
        'BARCODE'=>$r['Barcode'],
        'PROD'=>$r['PRODUCTO'], 
        'C'=>$cajas, 
        'U'=>$unds, 
        'CANT_ORIGINAL'=>$r['CANTIDAD'],
        'VAL'=>$r['CANTIDAD'] * $r['VALORPROD']
    ];
...
<head>
    ...
    <style>
        *{ box-sizing:border-box; }
        ...
        .item-row{ display:grid; grid-template-columns: 30px 1fr 50px 50px 85px; align-items:center; padding:8px 0; border-bottom:1px solid #f4f4f4; cursor:pointer; transition:.2s; }
        ...
        .btn-audit{ margin-top:25px; width:100%; background:#f57c00; color:white; border:none; border-radius:8px; padding:16px; font-size:18px; font-weight:bold; cursor:pointer; }
        .movil-nav { display: none; }

        @media (max-width:768px){
            body{ padding:8px; }
            .grid-container{ display:block; }
            .card{ display: none; width:100%; min-height:calc(100vh - 240px); padding:15px; border-radius:0; border-top:8px solid #f57c00; margin-bottom: 15px; }
            ...
            .movil-nav { display: flex; justify-content: space-between; align-items: center; background: #fff; padding: 12px; border-radius: 8px; margin-top: 5px; margin-bottom: 15px; box-shadow: 0 2px 6px rgba(0,0,0,.08); }
            .movil-nav button { background: #333; color: white; border: none; padding: 12px 20px; font-size: 15px; font-weight: bold; border-radius: 6px; cursor: pointer; }
            .movil-nav button:disabled { background: #ccc; cursor: not-allowed; }
            .movil-nav span { font-size: 15px; font-weight: bold; color: #555; }
        }
    </style>
</head>
<body>
    ...
    <form action="procesar_auditoria.php" method="POST">
        <div class="grid-container" id="contenedorTarjetas">
            <?php 
            $fila_global = 0; // Índice único por cada fila de producto enviada
            foreach($pedidos as $nro => $d): 
            ?>
            <div class="card" data-index="<?= $fila_global ?>">
                ...
                <?php foreach($d['ITEMS'] as $i): ?>
                    <label class="item-row">
                        <input type="hidden" name="productos[<?= $fila_global ?>][sede]" value="<?= $d['SUCURSAL'] ?>">
                        <input type="hidden" name="productos[<?= $fila_global ?>][nro_pedido]" value="<?= $nro ?>">
                        <input type="hidden" name="productos[<?= $fila_global ?>][facturador]" value="<?= htmlspecialchars($d['FACTURADOR']) ?>">
                        <input type="hidden" name="productos[<?= $fila_global ?>][barcode]" value="<?= $i['BARCODE'] ?>">
                        <input type="hidden" name="productos[<?= $fila_global ?>][cantidad_facturada]" value="<?= $i['CANT_ORIGINAL'] ?>">

                        <input type="checkbox" name="productos[<?= $fila_global ?>][checked]" value="1">
        ...
        <input type="hidden" name="pedido_activo_celular" id="pedidoActivoCelular" value="">
    ...
    <script>
        ...
        function inicializarPaginacion() {
            if (window.innerWidth <= 768 && tarjetas.length > 0) {
                ...
                document.getElementById('pedidoActivoCelular').value = ""; // En PC se procesa todo
            }
        }
        ...
        function actualizarControles() {
            ...
            // Guardamos el número de documento actual para procesar solo este en móviles
            let cabeceraTexto = tarjetas[indexActual].querySelector('.card-header').innerText;
            let nroDoc = cabeceraTexto.split('|')[0].replace('Doc:', '').trim();
            document.getElementById('pedidoActivoCelular').value = nroDoc;
        }
