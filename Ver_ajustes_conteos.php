...
<head>
    ...
    <style>
        ...
        .row-fade-out { opacity: 0; transform: scale(0.9); transition: all 0.4s ease; }
    ...
    <div class="row g-4">
            ...
            <div class="card">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="input-group input-group-sm" style="width: 250px;">
                        <input type="text" id="searchInput" class="form-control bg-light" placeholder="Filtrar...">
                        <button onclick="location.reload()" class="btn btn-outline-secondary"><i class="bi bi-arrow-clockwise"></i></button>
                    </div>
                </div>
                <div class="card-body p-0">
                    <table class="table align-middle mb-0" id="tablePendientes">
                        <thead>
                            <tr>
                                <th class="ps-4">CATEGORÍA / FECHA CONTEO</th>
                            ...
                            <tr id="fila-<?= $r['id'] ?>">
                                <td class="ps-4">
                                    ...
                                    <div class="text-primary fw-semibold" style="font-size: 10px;">
                                        <i class="bi bi-calendar3"></i> <?= date("d/m/Y H:i", strtotime($r['fecha_conteo'])) ?>
                                    </div>
                                    <div class="text-muted" style="font-size: 10px;">SEDE: <?= ($r['NitEmpresa'] === '901724534-7') ? 'DRINKS' : 'CENTRAL' ?></div>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <button onclick="procesarAccion(<?= $r['id'] ?>, 'ajustar_ajax')" class="btn btn-primary btn-sm px-3 rounded-start-pill" title="Ajustar">Ajustar</button>
                                        <button onclick="procesarAccion(<?= $r['id'] ?>, 'eliminar_ajax')" class="btn btn-danger btn-sm rounded-end-pill" title="Eliminar"><i class="bi bi-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endwhile; ?>
        ...
        <div class="col-lg-4">
            <div class="card h-100">
                <div class="card-header bg-white py-3 fw-bold">HOY: AJUSTES REALIZADOS</div>
                <div class="card-body p-0">
                    <div id="historialList" class="list-group list-group-flush" style="max-height: 600px; overflow-y: auto;">
                        <?php while($h = $resHistorial->fetch_assoc()): ?>
                            <div class="list-group-item border-0 border-bottom py-3">
                                ...
                                <div class="text-muted small">Diferencia aplicada: <strong><?= number_format($h['diferencia_aplicada'], 2) ?></strong></div>
                            </div>
                        <?php endwhile; ?>
                    </div>
                </div>
...
function procesarAccion(id, accion) {
    let confirmMsg = (accion === 'ajustar_ajax') ? "¿Aplicar ajuste al inventario real?" : "¿Eliminar este conteo de la lista?";
    ...
    .then(data => {
        ...
        if(data.status === 'success') {
            ...
            setTimeout(() => { 
                fila.remove(); 
                if(accion === 'ajustar_ajax') location.reload(); 
            }, 400);
        } else {
