        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $permitidas)) {
                ...
                if (move_uploaded_file($_FILES['foto']['tmp_name'], $ruta_foto)) {
                    $actualizarFotoSql = ", url_foto = ?";
                    $paramsFotoVal[] = $ruta_foto;
                }
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar_complementarios'])) {
    ...
    if (isset($_FILES['edit_foto']) && $_FILES['edit_foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $permitidas)) {
            ...
            if (move_uploaded_file($_FILES['edit_foto']['tmp_name'], $ruta_nueva)) {
                $sqlFotoUpdate = ", url_foto = ?";
                $paramsFoto[] = $ruta_nueva;
            }
        }
    }
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <button type="button" class="btn btn-link text-danger p-0 me-2" data-bs-toggle="modal" data-bs-target="#modalEmergencia<?= $c['id'] ?>" title="Contactos de Emergencia (Modal Interno)">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-8 d-flex align-items-end gap-2">
                            ...
                            <button type="button" class="btn btn-success btn-sm fw-bold text-white shadow-sm text-nowrap py-2" onclick="abrirPopupEmergencia(<?= $c['id'] ?>)" title="Crear Contacto de Emergencia vía Popup">
                                ➕ Emergencia (Popup)
                            </button>
                        </div>
                    </div>
