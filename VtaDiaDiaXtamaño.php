...
// Inicialización de filtros: 'Y-m-01' arranca desde el día 1 del mes en curso
...
/* =====================================================
    AGRUPACIÓN CRONOLÓGICA DIARIA (MES-DIA-NOMBRE)
===================================================== */
$ventasDiarias = [];
$totalCajasGlobal = 0;
$totalUndsGlobal = 0;
...
foreach ($rows as $r) {
    $anoRaw = substr($r['FECHA'], 0, 4);
    $mesRaw = substr($r['FECHA'], 4, 2);
    $diaRaw = substr($r['FECHA'], 6, 2);
    
    $timestamp = strtotime("$anoRaw-$mesRaw-$diaRaw");
    $nombreDiaIngles = date('l', $timestamp);
    $nombreDiaEs = $diasSemana[$nombreDiaIngles] ?? '';

    $fechaFormato = $mesRaw . '-' . $diaRaw . ' (' . $nombreDiaEs . ')';
    ...
    if (!isset($ventasDiarias[$fechaFormato])) {
        $ventasDiarias[$fechaFormato] = [
            'Cajas' => 0,
            'Unidades' => 0,
            'MontoTotal' => 0
        ];
    }
    $ventasDiarias[$fechaFormato]['Cajas'] += $cajas;
    $ventasDiarias[$fechaFormato]['Unidades'] += $unds;
    $ventasDiarias[$fechaFormato]['MontoTotal'] += $total_item;

    $totalCajasGlobal += $cajas;
    $totalUndsGlobal  += $unds;
    $granTotalGlobal  += $total_item;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .dashboard-row { display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px; }
        .dashboard-col-left { flex: 1.3; min-width: 400px; }
        .dashboard-col-right { flex: 1.7; min-width: 500px; background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 15px; height: 350px; position: relative; box-sizing: border-box; }
        .titulo-seccion { margin: 0 0 10px 0; font-size: 16px; color: #37474f; font-weight: bold; }
    </style>
...
<div class="card">
    ...
    <?php if(!$rows): ?>
        <p style="margin-top:20px; text-align:center; color:#777;">No se encontraron registros con los filtros seleccionados.</p>
    <?php else: ?>

        <div class="dashboard-row">
            
            <div class="dashboard-col-left">
                <h3 class="titulo-seccion">📅 Consolidado Diario</h3>
                <div class="table-container">
                    <table id="tablaResumenDiario">
                        <thead>
                            <tr>
                                <th>Fecha / Día</th>
                        ...
                        <tbody>
                            <?php foreach ($ventasDiarias as $fechaKey => $datosDia): ?>
                                <tr>
                                    <td><strong><?=$fechaKey?></strong></td>
                                    <td align="center"><?=$datosDia['Cajas']?></td>
                                    <td align="center"><?=$datosDia['Unidades']?></td>
                                    <td><strong>$ <?=number_format($datosDia['MontoTotal'], 0, ',', '.')?></strong></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        <tfoot>
                            <tr class="gran-total">
                                ...
                                <td>$ <?=number_format($granTotalGlobal, 0, ',', '.')?></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <div class="dashboard-col-right">
                <h3 class="titulo-seccion">📈 Tendencia de Venta Diario ($)</h3>
...
const labelsDias = <?= json_encode(array_keys($ventasDiarias)) ?>;
const dataMontos = <?= json_encode(array_column($ventasDiarias, 'MontoTotal')) ?>;
            ...
            tooltip: {
                callbacks: {
                    label: function(context) {
                        return ' Venta: $' + context.parsed.y.toLocaleString('es-CO');
                    }
                }
            }
        }
...
function exportarExcel() {
    let table = document.getElementById("tablaResumenDiario");
    if(!table) return;
    TableToExcel.convert(table, { 
        name: "Consolidado_Ventas_<?=date('Ymd_His')?>.xlsx",
        sheet: { name: "Consolidado Diario" }
    });
}
</script>
</body>
