...
$fCat       = $_GET['categoria'] ?? ''; // Nuevo filtro de categoría

$f_ini = str_replace('-', '', $f_ini_raw);
$f_fin = str_replace('-', '', $f_fin_raw);

// 1. CARGAR LISTA DE CATEGORÍAS PARA EL SELECTOR
$listaCategorias = [];
if (isset($mysqliWeb) && !$mysqliWeb->connect_error) {
    $qCat = $mysqliWeb->query("SELECT CodCat, DesCat FROM categorias ORDER BY DesCat ASC");
    if ($qCat) {
...
// 2. OBTENER UNICAJA Y CODCAT ASOCIADO DE LOS PRODUCTOS
$unicaja = [];
$catProductoMap = []; // Guarda a qué categoría pertenece el Sku
...
if ($skus && isset($mysqliWeb)) {
    ...
    $q = $mysqliWeb->query("SELECT cp.Sku, cat.Unicaja, cat.CodCat FROM catproductos cp INNER JOIN categorias cat ON cp.CodCat = cat.CodCat WHERE cp.Sku IN ($listaSkus)");
    if ($q) {
        while ($u = $q->fetch_assoc()) {
            $unicaja[$u['Sku']] = $u['Unicaja'];
            $catProductoMap[$u['Sku']] = $u['CodCat'];
        }
    }
}

// 3. FILTRAR EL ARRAY GENERAL EN PHP SI SE SELECCIONÓ UNA CATEGORÍA ESPECÍFICA
if ($fCat != '') {
    $rows = array_filter($rows, function($r) use ($catProductoMap, $fCat) {
        $sku = $r['Barcode'];
        return (isset($catProductoMap[$sku]) && $catProductoMap[$sku] == $fCat);
    });
}

// 4. PROCESAR EL RESUMEN DE PACAS VENDIDAS POR CAJERO
$resumenCajeros = [];
foreach ($rows as $r) {
    $cajero = $r['FACTURADOR'];
    $cant_total = $r['CANTIDAD'];
    $cajas = floor($cant_total); // Cantidad entera de pacas/cajas

    if (!isset($resumenCajeros[$cajero])) {
        $resumenCajeros[$cajero] = 0;
    }
    $resumenCajeros[$cajero] += $cajas;
}
arsort($resumenCajeros); // Ordenar de mayor a menor ventas
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .resumen-title { color: #37474f; border-bottom: 2px solid #0288d1; padding-bottom: 5px; margin-bottom: 15px; }
    </style>
        ...
        <div class="filter-group">
            ...
            <select name="categoria">
                ...
                <?php foreach($listaCategorias as $cat): ?>
                    <option value="<?=htmlspecialchars($cat['CodCat'])?>" <?=$fCat==$cat['CodCat']?'selected':''?>><?=htmlspecialchars($cat['DesCat'])?></option>
                <?php endforeach; ?>
            </select>
        </div>
...
<?php if(!empty($resumenCajeros)): ?>
<div class="card" style="max-width: 500px;">
    <h3 class="resumen-title">📦 Pacas Vendidas por Cajero</h3>
    <table>
        <thead>
            <tr style="background: #37474f; color: white;">
                <th style="padding: 8px;">Cajero / Facturador</th>
                <th style="padding: 8px; text-align: center;">Total Pacas (Cajas)</th>
            </tr>
        </thead>
        <tbody>
            <?php 
            $totalPacasResumen = 0;
            foreach($resumenCajeros as $cajero => $pacas): 
                $totalPacasResumen += $pacas;
            ?>
            <tr>
                <td><?=htmlspecialchars($cajero)?></td>
                <td align="center" style="font-weight: bold; color: #1b5e20;"><?=number_format($pacas, 0)?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
        <tfoot>
            <tr style="background: #cfd8dc; font-weight: bold;">
                <td style="padding: 8px;">TOTAL PACAS:</td>
                <td align="center" style="padding: 8px;"><?=number_format($totalPacasResumen, 0)?></td>
            </tr>
        </tfoot>
    </table>
</div>
